<template>
  <v-container>
    ...
    <div class="mb-2">
      ...
      <el-tooltip effect="dark" content="Удалить выделенные страны">
      ...
      <template v-for="(fruit, index) in LangFormThead">
        ...
        >
          <template slot-scope="scope">
            ...
            <v-checkbox
              v-else-if="fruit.nameField==='Visible'"
              readonly
              v-model="scope.row[fruit.nameField]"
...
export default {
  ...
  data() {
    return {
      ...
      formThead: [
        ...
        { nameField: "Visible", lngName: "Видимость", chkbD: true }
      ],
  ...
  computed: {
    ...
    prGetList: {
      get() {
        if (this.$store.getters["countries/getPrGetList"]) {
          this.getList();
        }
        return this.$store.getters["countries/getPrGetList"];
      },
      set(newValue) {
        this.$store.dispatch("countries/setPrGetList", newValue);
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    handleInsertItem() {
      this.$store.commit("countries/RESET");
      this.$store.commit("countries/SET_DIALOG_FORM", true);
    },
    ...
    editItem(item) {
      this.$store.commit("countries/SET_COUNTRIE", Object.assign({}, item));
      this.$store.commit("countries/SET_DIALOG_FORM", true);
    },
    deleteItem(item) {
